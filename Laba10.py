...
#Код, который размещает окно с игрой в середине экрана
...
def check_line(a1,a2,a3,smb):
    if a1['text'] == smb and a2['text'] == smb and a3['text'] == smb:
        ...
        global game_run
        game_run = False
        messagebox.showinfo(title="УВЕДОМЛЕНИЕ", message="Игра закончена.")
...
for row in range(3):
    ...
    for col in range(3):
        ...
        line.append(button)
    field.append(line)
new_button = Button(root,cursor = "hand2", relief = RAISED, activeforeground = "red", text='НОВАЯ ИГРА', command=new_game)
